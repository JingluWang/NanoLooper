...
//Adding
	...
	for(unsigned int igen=0;igen<nt.GenPart_p4().size();igen++){
		...
		ana.tx.pushbackToBranch<LorentzVector>	("Common_gen_p4s", nt.GenPart_p4()[igen]);                                           // Selected gen-particle
		ana.tx.pushbackToBranch<int>	  	("Common_gen_status",nt.GenPart_status()[igen]);
		ana.tx.pushbackToBranch<int>		("Common_gen_statusFlags",nt.GenPart_statusFlags()[igen]);
 	}
    }
...
{
    if (ana.cutflow.getCut("LastCutHere").pass){
                   ana.tx.fill(); 
    }
    return;
}	
