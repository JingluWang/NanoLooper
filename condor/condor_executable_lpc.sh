...
COPY_SRC="`pwd`/${OUTPUTNAME}.root"
COPY_DEST="root://cmseos.fnal.gov//${OUTPUTDIRPATHNEW}/${OUTPUTNAME}_${IFILE}.root"
xrdfs root://cmseos.fnal.gov mkdir -p ${OUTPUTDIRPATHNEW}
xrdcp ${COPY_SRC} ${COPY_DEST}
...
#COPY_DEST="davs://root://cmseos.fnal.gov//${OUTPUTDIRPATHNEW}/${OUTPUTNAME}_${IFILE}.root"
#echo "Running: env -i X509_USER_PROXY=${X509_USER_PROXY} gfal-copy -p -f -t 4200 --verbose --checksum ADLER32 ${COPY_SRC} ${COPY_DEST}"
#env -i X509_USER_PROXY=${X509_USER_PROXY} gfal-copy -p -f -t 4200 --verbose --checksum ADLER32 ${COPY_SRC} ${COPY_DEST}
#COPY_STATUS=$?
#if [[ $COPY_STATUS != 0 ]]; then
